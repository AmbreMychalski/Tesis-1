...
path_dataset = 'Front_React/documents/'
...
rawDataset = "Front_React/documents/"
txt_directory = "Back_Python/ProcessedDataset/txt/"
scraped_directory = "Back_Python/ProcessedDataset/scraped/"
embeddings_directory = "Back_Python/embeddings/"
...
def extract_text_with_coordinates():
    ...
    df.to_csv(scraped_directory+'scraped_raw_remaniement.csv')
    return df
...
def make_chunks(df):
    ...
    df2.to_csv(scraped_directory+'scraped_chunks_remaniement.csv')
    return df2
...
def extract_references(df):
    ...
    df.to_csv(scraped_directory+'scraped_without_ref_remaniement.csv')
    return df
...
def make_traductions(df):
    ...
    df['n_tokens'] = df.text.apply(lambda x: len(tokenizer.encode(x)))
    df.to_csv(scraped_directory+'shorteneds_remaniement.csv')
    return df
...
def make_embed(df):
    ...
    df.to_csv(embeddings_directory+'embeddings_chunks1500_remaniement.csv')
...
if __name__ == '__main__':
    ...
    # df_complete = pd.read_csv(embeddings_directory+'embeddings_chunks1500.csv')
